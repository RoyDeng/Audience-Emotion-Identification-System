    ...
    public class UserController : Controller
    {
        ...
        public ActionResult MyProductDetail(int ProductID)
        {
            if (Session["username"] != null)
            {
                ...
                using (MySqlConnection con = new MySqlConnection(constr))
                {
                    string query = string.Format("SELECT p.product_id, category_id, name, description, quantity, price, title, number, date_start, date_end, viewed FROM video AS v INNER JOIN product AS p ON v.product_id=p.product_id WHERE p.product_id=@product_id");
                    using (MySqlCommand cmd = new MySqlCommand(query))
                    {
                        cmd.Connection = con;
